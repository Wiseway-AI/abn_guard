  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav><p className="nav-title">Workspace</p>{nav.map((item) => <button key={item.id} className={tab === item.id ? "nav-item active" : "nav-item"} onClick={() => navigateTo(item.id)}><span>{item.icon}</span><div><b>{item.label}</b><small>{item.hint}</small></div></button>)}<button className={tab === "settings" ? "nav-item mobile-only-nav active" : "nav-item mobile-only-nav"} onClick={() => navigateTo("settings")}><span>⚙</span><div><b>Settings</b><small>Connection</small></div></button>{isCloudAccount(currentAccount) && <button className="nav-item mobile-only-nav" type="button" disabled={billingBusy} onClick={() => void (currentAccount.plan === "starter" ? openBillingPortal() : startCheckout("starter"))}><span>$</span><div><b>{billingBusy ? "Opening…" : currentAccount.plan === "starter" ? "Manage plan" : "Upgrade"}</b><small>Stripe billing</small></div></button>}<button className="nav-item mobile-only-nav" type="button" onClick={signOut}><span>↗</span><div><b>Sign out</b><small>{currentAccount.email}</small></div></button></nav>
