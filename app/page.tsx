  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav><p className="nav-title">Workspace</p>{nav.map((item) => <button key={item.id} className={tab === item.id ? "nav-item active" : "nav-item"} onClick={() => navigateTo(item.id)}><span>{item.icon}</span><div><b>{item.label}</b><small>{item.hint}</small></div></button>)}<button className={tab === "settings" ? "nav-item mobile-only-nav active" : "nav-item mobile-only-nav"} onClick={() => navigateTo("settings")}><span>⚙</span><div><b>Settings</b><small>{apiConfigured ? "Official API" : "Demo mode"}</small></div></button></nav>
