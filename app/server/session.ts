import { getUserWorkspace } from "./database";
...
export async function createSessionCookie(userId: string, request: Request) {
  const payload: SessionPayload = { userId, exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 14 };
...
export async function sessionFromRequest(request: Request) {
  try {
    ...
    return getUserWorkspace(payload.userId);
  } catch {
    return null;
  }
}
